...
String getFullImageUrl(String url) {
  ...
  return "${AppConfig.serverOrigin}$url";
}
...
class _TeacherAchievementPageState extends State<TeacherAchievementPage> {
  ...
  Future<List<Achievement>> fetchAchievements() async {
    ...
    final response = await http.get(
      Uri.parse("${AppConfig.baseUrl}/achievements/visible"),
      headers: {'accept': 'application/json', 'Authorization': 'Bearer $token'},
    );

    if (response.statusCode == 200) {
      final List jsonData = json.decode(response.body);
      final achievements = jsonData
                    ...
                    ElevatedButton.icon(
                      ...
                      onPressed: () {
                        Navigator.push(
                          ...
                          MaterialPageRoute(
                            builder: (context) =>
                                const AddTeacherAchievementPage(),
                          ),
                        );
                      },
